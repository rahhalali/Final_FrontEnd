...
import {time, WeatherAsync} from "../../Redux/TimerWeather";

function OverView(props) {
    const timer = useSelector(state=>state.weathertime);

    const dispatch=useDispatch();
    useEffect(()=>{
        AOS.init({duration:1500});
        dispatch(WeatherAsync());
        let timer = setInterval(()=>dispatch(time()), 1000 )
        ...
        <>
            ...
            <div className='overview'>
                <div className="overview-content">
                    ...
                    <div className="info">
                        <h3 className='beirut' data-aos="fade-down">Styled rooms and suites, restaurants and spa, 6-star
                            luxury in Beirut</h3>
                        <p className='info-about' data-aos="fade-down">A Beirut landmark, LE ROYAL Hotel is within
                            walking distance to a host of art,
                        ...
                        <div className="direction" data-aos="fade-left">
                            ...
                            <div className='weather'>
                                <h3>NOW IN BEIRUT <span className='celsius'>{timer.weathertime &&
                                    timer.weathertime.main.temp}&deg;C</span>
                                    <span className='condition'>{timer.weathertime && timer.weathertime.weather[0].main}</span>
                                    {timer.time}</h3>
                            </div>
                        </div>
                    </div>
